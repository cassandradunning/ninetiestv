    ...
    class Program
    {
        ...
        **************************************************************************************************/
        ...
        static List<Show> ShowsWithAmpersand(List<Show> shows)
        {
            ...
            //OR
            //List<Show> showsWithAnd = shows.Where(!s => s.Name.Contains("&")).ToList()
            //return shows.Except(showsWithAnd).ToList();
        ...
        static List<Show> AllButWorst(List<Show> shows)
        {
            ...
            //or
            // return shows.Except(shows.OrderBy(s => s.ImdbRating).Take(1)).ToList();
            //return shows.OrderBy(s => s.ImdbRating).FirstOrDefault();
        ...
        static List<Show> WithMultipleGenresByStartYear(List<Show> shows)
        {
            ...
            // OR
            //  return shows.Where(s => s.Genres.Count() > 1).ToList();
        ...
        {
            return shows.Count(s => s.Genres.Contains("Crime") && s.ImdbRating > 7);
        }
        ...
        static Show WordieastName(List<Show> shows)
        {
            ...
            // splits the name into sub strings based on character separated
            // counts the individual strings and lists the first of the collection
        }
        ...
        static string AllNamesWithCommas(List<Show> shows)
        {
            ...
            // use string method to join together a comma and space
        }
